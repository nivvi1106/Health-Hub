...
con=sqlcon.connect(host="localhost",user="root",passwd="")#connection to mysql 
cur=con.cursor()
...
def entry():
    ...
    p5=e5.get()
    p6=e6.get()
    query='insert into patient values("{}", "{}", "{}", "{}", "{}", "{}")'.format(p1,p2,p3,p4,p5,p6)
    cur.execute(query)
    con.commit()
...
def get_apoint():
    global x1,x2,x3,x4
    p1=x1.get()  
    cur.execute('select * from patient where idno=(%s)',(p1,))
    dat=cur.fetchall()
    a=[]
    for i in dat:
        a.append(i)   
...
def do_modify():
    ...
    new=x5.get()
    if choice=='1':
        cur.execute('update patient set name={} where idno={}'.format(new,ad))
        con.commit()
    elif choice=='2':
        cur.execute('update patient set age={} where idno={}'.format(new,ad))
        con.commit()
    elif choice=='3':
        cur.execute('update patient set gender={} where idno={}'.format(new,ad))
        con.commit()
    elif choice=='4':
        cur.execute('update patient set phone={} where idno={}'.format(new,ad))
        con.commit()
    elif choice=='5':
        cur.execute('update patient set bg={} where idno={}'.format(new,ad))
        con.commit()
    else:
        pass
    root6.destroy()
...
root=Tk()
label=Label(root,text="XYZ HOSPITAL",font="arial 40 bold",bg='light blue')
